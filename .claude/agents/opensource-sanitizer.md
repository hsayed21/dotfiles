...
- Scan every file for REDACTED patterns, PII, and internal references
...
```
# API keys
pattern: [A-Za-z0-9_]*(api[_-]?key|apikey|api[_-]?REDACTED)[A-Za-z0-9_]*\s*[=:]\s*['"]?[A-Za-z0-9+/=_-]{16,}
...
# Internal REDACTED file references
pattern: \.secrets/
...
git log -p | grep -iE '(password|REDACTED|api.?key|token)' | head -20
```
...
- **Never** display full REDACTED values — truncate to first 4 chars + "..."
